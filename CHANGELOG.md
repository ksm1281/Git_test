...
### Змінено
- **modules/stock.php**: повна міграція на Alpine.js — всі 4 вкладки (Товари, Корекція, Ціни, Категорії) тепер використовують єдиний компонент `stockPage`
  - Категорії: CRUD-модалка, кнопка синхронізації з OpenCart зі спінером
  - Ціни: per-row редагування через `data-*` атрибути, чекбокси для масового push, індикатор `pushPrices`
  - Корекції: edit-модалка, batch-delete, кнопка "Нова корекція" відкриває `adjustModal` (виправлено баг — кнопка вела на іншу сторінку)
  - Товари: edit-модалка з multi-select категорій, sync (всі/один), adjust-modal з пошуковим автокомплітом
- **includes/footer.php**: зареєстровано Alpine-компонент `stockPage` поряд з `itemsForm` — централізовано всі методи (modal openers, sync, search, select, toggleAll)
- **modules/stock.php**: видалено дублікат `editProductModal` (був мертвий код), прибрано inline `<script>` блоки з IIFE-автокомплітом
